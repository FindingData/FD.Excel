...
namespace FD.Excel.Checker {
	public class ExcelChecker
	{
		...
		public ExcelChecker(string fileName, Stream stream) : this()
		{
		...
		private TemplateRule _templateRule
		{
			get; set;
		}
		...
		{
			if (_checkReport.CheckErrorList.Any())
			{
				return;
			}
			InitExcelStructure();
		...
		{
			foreach (var aSheet in _templateRule.SheetRules)
			{
				if (_excelStructure.Sheets.Any(f => f.SheetName == aSheet.SheetName))
				{
					var unMatchColumns = _excelStructure[aSheet.SheetName].Columns
						   .Where(c => !aSheet.ColumnRules.Any(a => a.ColumnName == c.ColumnName))
						   .Select(c => c.ColumnName)
						   .ToList();
					if (!unMatchColumns.Any())
						continue;
					var message = $"----模版[{aSheet.SheetName}]不包含列:[{ConvertListToString(unMatchColumns, "]、[")}]与工作表《{_excelStructure.Name}》.[{aSheet.SheetName}]，不匹配！";
					_checkReport.TemplateMatchError("匹配列失败", message);
					return false;
				}
				else
				{
					var message = $"----Excel《{_excelStructure.Name}》中没有找到与[{aSheet.SheetName}](共{aSheet.ColumnNum}列)匹配的工作表，不匹配!";
		...
		private void CheckDataSet()
		{
			...
			{
				if (!_templateRule.SheetRules.Any(t => t.SheetName == aTable.TableName))
					continue;
				...
				foreach (var aColumnStructure in aSheetStructure.Columns)
				{
					...
					if (aColumnRule.MeanDataType.ToLower() == "varchar2")
					{
						iRowNum = aTable.Select(String.Format("len([{0}]) > {1} ", aColumnRule.ColumnName, aColumnRule.FieldLength)).Count();
						if (iRowNum > 0)
						{
					...
					if (_fromSingleDic.Contains(aColumnRule.MeanDataType.ToLower()))
					{
						...
						foreach (var aParName in columnValues)
						{
							...
							else
							{
								...
								_checkReport.ColumnError(ErrorKey, $"----[字典检测]不通过：“{aParName}”对应字典项出现问题，共有{iRowNum}为该值，请联系管理员！");
							}
